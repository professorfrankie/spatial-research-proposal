...
fml1A <- reformulate(c("m1_hat", rhs_controls), response = "forest_loss_all_gross")
# Convert formula to character, then append fixed effects
rhs1A <- paste(all.vars(fml1A)[-1], collapse = " + ")
full_formula1A <- as.formula(paste("forest_loss_all_gross ~", rhs1A, "|muni_id + year"))
# Estimate fixed effects model
...
fml2A <- reformulate(c("m2_hat", rhs_controls), response = "forest_loss_all_gross")
# Convert formula to character, then append fixed effects
rhs2A <- paste(all.vars(fml2A)[-1], collapse = " + ")
full_formula2A <- as.formula(paste("forest_loss_all_gross ~", rhs2A, "| year"))
# Estimate fixed effects model
...
fml3A <- reformulate(c("m3_hat", rhs_controls), response = "forest_loss_all_gross")
# Convert formula to character, then append fixed effects
rhs3A <- paste(all.vars(fml3A)[-1], collapse = " + ")
full_formula3A <- as.formula(paste("forest_loss_all_gross ~", rhs3A, "| year"))
# Estimate fixed effects model
...
fml4A <- reformulate(c("m4_hat", rhs_controls), response = "forest_loss_all_gross")
# Convert formula to character, then append fixed effects
rhs4A <- paste(all.vars(fml4A)[-1], collapse = " + ")
full_formula4A <- as.formula(paste("forest_loss_all_gross ~", rhs4A, "| year"))
# Estimate fixed effects model
